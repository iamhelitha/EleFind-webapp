...
export default function NearestDetectionsPanel({
  ...
  if (detectionsWithDistance.length === 0) {
    return (
      <div className="rounded-lg border border-[rgba(32,30,29,0.14)] p-4 text-center">
        <p className="text-xs text-[rgba(32,30,29,0.55)]">No detections available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center gap-1.5 px-1">
        <Navigation className="h-3.5 w-3.5 text-accent-700" />
      ...
      {detectionsWithDistance.map(({ detection, distance }) => (
        <button
          ...
          className="flex w-full items-center gap-2.5 rounded-lg border border-[rgba(32,30,29,0.14)] p-2.5 text-left hover:border-sage-300 hover:bg-[rgba(32,30,29,0.06)]/50 transition-colors"
        >
          <span
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] text-[rgba(32,30,29,0.55)]">
              {new Date(detection.detectedAt).toLocaleDateString()} ·{" "}
              {(detection.confidence * 100).toFixed(0)}% conf.
            </p>
          </div>
          <div className="flex items-center gap-1 shrink-0">
            <MapPin className="h-3 w-3 text-[rgba(32,30,29,0.55)]" />
            <span className="text-xs font-medium text-accent-700 tabular-nums">
